...
```css
/* === 品牌主色 === */
--primary: #2261F5;          /* 品牌蓝：核心强调，CTA，关键数据高亮 */
--primary-light: #5585F8;    /* 浅品牌蓝：次级强调，hover状态 */
--primary-dark: #1A4DC4;     /* 深品牌蓝：pressed状态，深色背景 */

/* === 语义色 === */
--success: #00B96B;           /* 绿色：正向指标，增长 */
--warning: #F5A623;           /* 黄色：预警，需关注指标 */
--danger: #F5622D;            /* 橙红色：负向指标，下降 */
...
/* === 图表专用色序（10色，对应参考图新版颜色，色盲安全）===
   按视觉权重和色相环分布排列 */
--chart-1: #2261F5;   /* 品牌蓝    - 主系列，最重要数据 */
--chart-2: #2DCB8D;   /* 翠绿      - 次系列 */
--chart-3: #F5C518;   /* 明黄      - 三级系列 */
--chart-4: #F5622D;   /* 橙红      - 四级系列 */
--chart-5: #7B61FF;   /* 品牌紫    - 五级系列 */
--chart-6: #00BDD6;   /* 青蓝      - 六级系列 */
--chart-7: #8899B2;   /* 蓝灰      - 七级系列 */
--chart-8: #8BC34A;   /* 黄绿      - 八级系列 */
--chart-9: #FF4081;   /* 玫红      - 九级系列 */
--chart-10: #B44FE8;  /* 亮紫      - 十级系列 */
```
...
">
  ...
  <div style="
    ...
    background: radial-gradient(circle, rgba(34,97,245,0.15) 0%, transparent 70%);
    border-radius: 50%;
  "></div>
  ...
  <div style="
    ...
    background: rgba(34,97,245,0.15); border: 1px solid rgba(34,97,245,0.3);
    color: #2261F5; padding: 4px 12px; border-radius: 20px;
  ...
  <div style="display: flex; gap: 12px; flex-wrap: wrap;">
    <!-- 摘要数据标签 -->
    <div style="
      background: rgba(34,97,245,0.06); border-radius: 8px;
      padding: 8px 16px; color: #1A1A1A; border: 1px solid rgba(34,97,245,0.15);
    ">
      <span style="font-size: 11px; color: #8C8C8C;">核心指标</span>
      <div style="font-size: 20px; font-weight: 700; color: #2261F5;">数值</div>
    </div>
    <!-- 可复用，添加更多标签 -->
  </div>
</header>
...
">
  ...
  <div style="
    ...
    height: 3px;
    background: linear-gradient(90deg, #2261F5, #5585F8);
  "></div>
  ...
  ">
    <span style="width: 6px; height: 6px; border-radius: 50%; background: #2261F5; display: inline-block;"></span>
    [指标名称]
  </div>
  ...
  <div style="display: flex; align-items: center; gap: 8px; margin-top: 8px;">
    ...
    <!-- 负向趋势（切换使用）-->
    <!-- <span style="background: rgba(245,98,45,0.1); color: #F5622D; ...">▼ -5.2%</span> -->
  </div>
...
**KPI卡片顶部色条配色方案：**
- 蓝色（流量类）：`#2261F5 → #5585F8`
- 橙色（营收类）：`#F5622D → #FF7A5C`
- 绿色（增长类）：`#2DCB8D → #52D9A6`
- 紫色（用户类）：`#7B61FF → #9C87FF`
...
// 全局主题配置（所有图表共用）
const kwaiboolTheme = {
  color: ['#2261F5', '#2DCB8D', '#F5C518', '#F5622D', '#7B61FF', '#00BDD6', '#8899B2', '#8BC34A', '#FF4081', '#B44FE8'],
...
```html
<div style="
  background: linear-gradient(135deg, rgba(34,97,245,0.04) 0%, rgba(34,97,245,0.08) 100%);
  border-left: 4px solid #2261F5;
...
```css
/* 蓝色连续（流量/规模类指标）*/
--seq-blue-0: rgba(34, 97, 245, 0.08);   /* 极低 */
--seq-blue-1: rgba(34, 97, 245, 0.20);
--seq-blue-2: rgba(34, 97, 245, 0.40);
--seq-blue-3: rgba(34, 97, 245, 0.65);
--seq-blue-4: rgba(34, 97, 245, 0.85);
--seq-blue-5: #2261F5;                    /* 极高 */

/* 橙红连续（热度/活跃类指标）*/
--seq-orange-0: rgba(245, 98, 45, 0.08);
--seq-orange-5: #F5622D;
```
...
/* 正负发散色板（环比/同比偏差）*/
/* 负极端 → 中性 → 正极端 */
/* #F5622D → #FFF2EE → #FFFFFF → #F0FFF8 → #2DCB8D */
```
...
**主题 B：Light（浅色清晰版）** —— **默认**，适合文档输出/打印/日常周报
```css
.theme-light {
  --bg-page: #F5F5F5;
  --bg-card: #FFFFFF;
...
- 日常数据报告（默认）→ Light，清晰易读，适合各类屏幕和打印
- 详细数据分析报告 → Light，文字更易读
- 对外客户报告 → Light，避免打印变黑
- 大屏/投影演示/高管汇报 → 切换 Dark，对比度更高
