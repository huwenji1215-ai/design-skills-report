#!/usr/bin/env python3
"""
生成 FreeMind .mm 格式思维导图
兼容：XMind / MindNode / MindManager / OmniGraffle / Freeplane / 幕布导入
纯 XML，无需打包，最广泛兼容
"""
...
def skill_node(parent, s):
    color = P_COLOR.get(s["priority"], "#666666")
    note = (
        f"ID: {s['id']} | 优先级: {s['priority']} | "
        f"技术路线: {s['tech']} | 竞品对标: {s['benchmark']} | "
        f"2天可交付: {s['deliverable']} | 描述: {s['desc']}"
    )
    n = ET.SubElement(parent, "node", {
        "TEXT": f"[{s['priority']}] {s['label']}",
        "COLOR": color,
        "STYLE": "bubble",
    })
    ET.SubElement(n, "richcontent", {"TYPE": "NOTE"}).text = note
    return n

def group_node(parent, g, color="#555555"):
    label = g["label"]
    if g.get("sub"):
...
def direction_node(parent, d, position="right"):
    DIR_COLOR = {
        "ui":   "#388bfd",
        "img":  "#3fb950",
        "vid":  "#da3633",
    ...
    color = DIR_COLOR.get(d["id"], "#58a6ff")
    n = ET.SubElement(parent, "node", {
        "TEXT": f"{d['label']} [{d['sub']}]",
        "COLOR": color,
        "STYLE": "bubble",
        "FOLDED": "false",
        "POSITION": position,
        ...
        "children": [
            ...
            },
            {
                ...
                "children": [
                    ...
                    {"id":"IMG-D3","label":"图生图（风格迁移）","priority":"P1","deliverable":"yes","tech":"GPT-4o Image / SD img2img","benchmark":"Adobe Firefly","desc":"原图+改写指令->保留结构改变风格"},
            ...
            },
            {
                ...
                "children": [
                    {"id":"IMG-F1","label":"图片裂变·多尺寸适配","priority":"P1","deliverable":"yes","tech":"主体检测+Outpainting+批量导出","benchmark":"Adobe Express / 即时设计","desc":"一张主图->自动裁切/补全为App Banner/方图/微信封面/抖音竖版等多尺寸"},
                ]
            },
        ]
    },
        ...
        "children": [
            ...
            },
            {
                ...
                "children": [
                    ...
                    {"id":"VID-G2","label":"Figma->Lottie 一键导出","priority":"P1","deliverable":"yes","tech":"Magic Animator Beta（2025.07）","benchmark":"Magic Animator/Lottielab","desc":"Figma设计稿一键添加动效并导出Lottie JSON/MP4/GIF"},
                ]
            },
            {
            ...
            },
            {
                ...
                "children": [
                    ...
                    {"id":"VID-I2","label":"图生视频（设计稿动起来）","priority":"P2","deliverable":"warn","tech":"Kling 3.0 / Veo 3.1 API","benchmark":"Kling 3.0","desc":"设计稿截图->加自然运动的展示视频，支持4K@60fps最长20s"},
                ]
            },
        ]
    },
        ...
        "children": [
            {
                ...
                "children": [
                    {"id":"DOC-J1","label":"PPT 美化/视觉优化","priority":"P0","deliverable":"yes","tech":"LLM + PptxGenJS","benchmark":"Beautiful.ai / Gamma","desc":"输入已有PPT，大幅提升视觉层次、字体规范、色彩体系，输出可编辑.pptx"},
                    {"id":"DOC-J2","label":"PPT 生成（信息架构型）","priority":"P1","deliverable":"yes","tech":"LLM + 结构化模板","benchmark":"Skywork Agent","desc":"输入主题/大纲，生成逻辑严谨的PPT，符合金字塔原理/SCQA结构"},
                    {"id":"DOC-J3","label":"PPT 生成（美学表现型）","priority":"P1","deliverable":"warn","tech":"LLM + AI配图 + 模板","benchmark":"Gamma / 张月光系","desc":"输入主题，生成高颜值PPT，每页配图>=30%，适合对外展示/品牌提案"},
                    {"id":"DOC-J4","label":"PPT图片化输出（不可编辑）","priority":"P1","deliverable":"yes","tech":"HTML/CSS + Puppeteer截图","benchmark":"无直接竞品","desc":"输入内容->HTML排版->Puppeteer逐页截图->PDF/PNG打包，适合直接分发/社媒分享"},
                ]
            },
            {
                "label": "K. UX研究报告类", "sub": "可用性/体验/满意度",
                "children": [
                    {"id":"DOC-K1","label":"竞品分析文档生成","priority":"P1","deliverable":"yes","tech":"搜索 + LLM 结构化分析","benchmark":"无直接竞品","desc":"输入竞品名称，自动调研生成功能矩阵+截图说明+差异洞察"},
                    {"id":"DOC-K2","label":"可用性分析报告","priority":"P1","deliverable":"yes","tech":"LLM + Nielsen启发式","benchmark":"无直接竞品","desc":"输入用户研究数据，生成含P0-P2问题分级+用户原话引用+改进建议的可用性报告"},
                    {"id":"DOC-K3","label":"用户体验地图（UX Map）","priority":"P1","deliverable":"yes","tech":"LLM + SVG可视化输出","benchmark":"无直接竞品","desc":"输入用户旅程描述/访谈数据，生成各触点情绪曲线+痛点+机会点的可视化UX Map"},
        ...
        "children": [
            {
                ...
                "children": [
                    ...
                    {"id":"EVAL-M2","label":"视觉规范合规评测","priority":"P1","deliverable":"yes","tech":"颜色/字体/间距提取+Token比对","benchmark":"内部 Design Token 规范","desc":"检查UI是否符合Design Token规范，输出违规点清单+合规率百分比"},
            ...
            },
            {
                ...
                "children": [
                    ...
                    {"id":"EVAL-P1","label":"自动化设计走查","priority":"P1","deliverable":"warn","tech":"规则引擎 + 多模态视觉","benchmark":"内部走查规则集","desc":"输入设计稿+走查规则，自动执行视觉规范/可用性/无障碍/业务逻辑四类走查"},
...
root_node = ET.SubElement(map_el, "node", {
    "TEXT": "设计 Skill 体系",
    "COLOR": "#ffffff",
...
positions = ["right", "right", "right", "left", "left"]
for i, d in enumerate(ALL_DATA):
    direction_node(root_node, d, position=positions[i])
...
# ── 写入 .mm 文件 ──────────────────────────────────────────
mm_path = "设计Skill体系.mm"
with open(mm_path, "w", encoding="utf-8") as f:
    f.write(xml_str)

size = os.path.getsize(mm_path)
print(f"✅ 已生成: {os.path.abspath(mm_path)}")
print(f"   格式:     FreeMind .mm (标准XML，最广泛兼容)")
print(f"   文件大小: {size:,} bytes")
print(f"   打开方式: XMind / MindNode / Freeplane / MindManager 均支持导入")

# 验证 XML
import xml.etree.ElementTree as ET2
tree = ET2.parse(mm_path)
nodes = tree.findall(".//node")
print(f"   节点数量: {len(nodes)} 个节点 ✅")
