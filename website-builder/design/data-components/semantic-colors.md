...
> ⚠️ **两套颜色体系，不得混用：**  
> - **页面语义色**（本文件 `--semantic-xxx`）→ 涨跌 Delta / 状态 Badge / 进度条颜色  
> - **图表多系列色**（`data-components/echarts-config.md` `CHART_PALETTE`）→ 折线/柱/饼图各系列区分  
>
> 两套体系颜色刻意错开，避免图表系列色与涨跌语义色产生视觉混淆。  
> 深色主题（B/F）中语义色使用更亮版本（见各自主题文件）。
